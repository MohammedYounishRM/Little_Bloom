...
const API_URL = import.meta.env.VITE_API_BASE_URL ? `${import.meta.env.VITE_API_BASE_URL}/api/auth` : "http://localhost:5000/api/auth";
...
export const useAuthStore = create((set) => ({
    ...
    signup: async (name, email, password, phone, center_name, center_id) => {
        set({isLoading: true, error: null});
        try {
            const response = await axios.post(`${API_URL}/signup`,{ name, email, password, phone, center_name, center_id });
    ...
    updateProfile: async (profileData) => {
        ...
        try {
            ...
            set({ error: error.response.data.message || "Error updating profile details.", isLoading: false });
            throw error;
        }
    }
}));